  ...
  const findUsers = useCallback(
    async (players) => {
      ...
      const sessionData = JSON.parse(
        sessionStorage.getItem('osrs_highscores_searched_users') || "[]"
      );
        ...
        try {
          ...
          if (validResults.length > 0) {
            ...
            setPlayersData((prevData) => {
              ...
              const newPlayers = validResults.filter((player) => 
                !existingUsers.includes(player.username)
              );
            ...
            sessionStorage.setItem(
              'osrs_highscores_searched_users',
